...
public class LoginScreen {
    ...
    public LoginScreen() throws IOException {
        ...
        emailL.setFont(new Font(Font.SERIF,Font.BOLD,34));

        passwordL=new JLabel("Email: ");
        passwordL.setFont(new Font(Font.SERIF,Font.BOLD,34));


        loginButton = new JButton("Forgot Password?") {
            protected void paintComponent(Graphics g) {
                if (!isOpaque() && getBorder() instanceof RoundedCornerBorder) {
                    Graphics2D g2 = (Graphics2D) g.create();
                    g2.setPaint(getBackground());
        ...
        jframe.setContentPane(new JPanel() {
            BufferedImage bufferedImage = ImageIO.read(Objects.requireNonNull(this.getClass().getResource("/backgroud.png")));
            protected void paintComponent(Graphics g) {
                super.paintComponent(g);
            ...
            public void removeUpdate(DocumentEvent e) {
                if(email.getText().length() > 0 && !email.getText().equals("Enter your email")) {
                    if(validateMail(email.getText())) {
                        usernameError.setForeground(new Color(0,255,0));
    ...
    public void init() {
        email.setPreferredSize(new Dimension(300,45));
        ...
        cancel.setBackground(new Color(0, 255, 0));
        cancel.setFocusPainted(false);

        next.setPreferredSize(new Dimension(150,50));
        next.setBackground(new Color(66, 245, 114));
        ...
        Insets emailInsets = new Insets(20, 5, 5, 10);

        Insets errorInsets = new Insets(0,20,0,0);

        Insets buttonInsets = new Insets(5, 160, 5, 5);
        Insets cancelInsert = new Insets(20,170,50,10);
        Insets nextInsert = new Insets(20,10,50,5);
        ...
        input.insets = cancelInsert;
        input.anchor = GridBagConstraints.WEST;
        input.gridx = 0;
        input.gridy = 6;
        jframe.add(cancel,input);

        input.insets = nextInsert;
        input.anchor = GridBagConstraints.WEST;
        input.gridx = (int) .5;
        input.gridy = 6;
        jframe.add(next,input);

        input.insets = emailInsets;
        input.anchor = GridBagConstraints.WEST;
        input.gridx = 0;
        input.gridy = 0;
        jframe.add(emailL,input);
